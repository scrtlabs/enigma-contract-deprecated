...
class GasTracker {
    ...
    logGasUsed(result, fn) {
        ...
        this.gasUsed.push(
            this.web3.eth.getTransaction(result.tx).then(tx => {
    ...
    displayStats() {
        return Promise.all(this.gasUsed).then(gasUsed => {
            console.log(
                "Cost of transactions based on gas price:",
                this.gasPrice,
                "gwei"
            );
...
class Principal {
    ...
    register() {
        ...
        return this.contract.methods.register(this.params[0], report).send({
            from: this.custodian,
            gas: 4712388,
    ...
    setWorkersParams() {
        ...
        if (engUtils.toAddress(signer) !== this.params[0])
            throw "invalid principal signature";

        console.log("updating workers parameters with seed", seed);
        return this.contract.methods.setWorkersParams(seed, sig).send({
            from: this.custodian,
            gas: 4712388,
            gasPrice: web3Utils.toWei(GAS_PRICE_GWEI, "gwei")
        });
